...
This report uses a stylized comparison between two surgeons to illustrate how a **common-cause confounder**—initial patient severity—can distort observational comparisons, and how **randomization** and **stratification** help recover interpretable causal conclusions. We present observational outcomes by patient, outcomes under randomized assignment, and a severity-stratified view of the same observational cohort, with directed acyclic graphs (DAGs) to clarify the causal structure at each step.
...
### Figure 1. Post-surgical outcomes by patient (observational cohort)

Doc Dreamy appears as blue circles and Doc Duck as red triangles; dashed horizontal lines and labels report each surgeon's cohort mean.
...
# --- Style constants (colorblind-friendly palette) ---
dreamy_color = "#4E79A7"
duck_color = "#E15759"
dreamy_marker = "o"
duck_marker = "^"
...
# --- Split by doctor (patients_df loaded in prior chunk) ---
is_dreamy = patients_df["doctor_name"] == "Doc Dreamy"
dreamy_patients = patients_df.loc[is_dreamy]
duck_patients = patients_df.loc[~is_dreamy]

mean_dreamy = dreamy_patients["post_surgical_score"].mean()
mean_duck = duck_patients["post_surgical_score"].mean()

# --- Figure ---
fig, ax = plt.subplots(figsize=(8, 6), layout="constrained")

ax.scatter(
    dreamy_patients["patient"],
    dreamy_patients["post_surgical_score"],
    c=dreamy_color,
...
ax.scatter(
    duck_patients["patient"],
    duck_patients["post_surgical_score"],
    c=duck_color,
    marker=duck_marker,
    s=65,
    edgecolors=point_edge,
...
annotate_x = float(patients_df["patient"].max()) - 2
ax.text(
    annotate_x,
    mean_dreamy + 0.15,
...
ax.set_title("Post-Surgical Symptom Score by Patient and Doctor", fontsize=13, pad=12)
...
### Figure 2. Post-surgical outcomes under randomized assignment

The chart below matches the styling of @fig-plot-outcomes but uses the randomized cohort, so severity is no longer aligned with surgeon assignment.
...
# --- Style constants (colorblind-friendly palette; match Figure 1) ---
dreamy_color = "#4E79A7"
duck_color = "#E15759"
dreamy_marker = "o"
duck_marker = "^"
...
# --- Split by doctor (patients_randomized_df loaded in prior chunk) ---
is_dreamy = patients_randomized_df["doctor_name"] == "Doc Dreamy"
dreamy_patients = patients_randomized_df.loc[is_dreamy]
duck_patients = patients_randomized_df.loc[~is_dreamy]
...
annotate_x = float(patients_randomized_df["patient"].max()) - 2
ax.text(
    annotate_x,
    mean_dreamy + 0.15,
    f"○ mean = {mean_dreamy:.2f}",
...
ax.set_ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=11)
ax.set_title(
    "Post-Surgical Symptom Score (Randomized Assignment)",
    fontsize=13,
    pad=12,
)
ax.grid(True, linestyle="-", alpha=0.28)
...
### Figure 3. Outcomes by initial patient severity (stratification)

A shaded band from $-1$ to $1$ on the severity axis marks the region emphasized in the narrative, where both surgeons have enough overlap for direct comparison.
...
# --- Style constants (colorblind-friendly palette; match Figures 1–2) ---
dreamy_color = "#4E79A7"
duck_color = "#E15759"
dreamy_marker = "o"
duck_marker = "^"
point_edge = "white"
point_linewidth = 0.6
stratify_band_color = "gray"
stratify_band_alpha = 0.15

# --- Split by doctor (observational patients_df loaded in prior chunk) ---
is_dreamy = patients_df["doctor_name"] == "Doc Dreamy"
dreamy_patients = patients_df.loc[is_dreamy]
duck_patients = patients_df.loc[~is_dreamy]
...
ax.axvspan(
    -1,
    1,
    facecolor=stratify_band_color,
    alpha=stratify_band_alpha,
    zorder=0,
    linewidth=0,
)
...
ax.set_ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=11)
ax.set_title(
    "Post-Surgical Symptom Score by Patient Severity",
    fontsize=13,
    pad=12,
)
ax.grid(True, linestyle="-", alpha=0.28)
...
annotate_x = x_max - 0.02 * (x_max - x_min)
ax.text(
    annotate_x,
    mean_dreamy + 0.15,
    f"○ mean = {mean_dreamy:.2f}",
